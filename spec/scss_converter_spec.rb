...
describe(Bunto::Converters::Scss) do
  let(:site) do
    Bunto::Site.new(site_configuration)
  end
  let(:content) do
    <<-SCSS
...
SCSS
  ...
  def converter(overrides = {})
    Bunto::Converters::Scss.new(site_configuration({"sass" => overrides}))
  end
  ...
  context "when building configurations" do
    ...
    context "in safe mode" do
      let(:verter) {
        Bunto::Converters::Scss.new(site.config.merge({
          "sass" => {},
          "safe" => true
        }))
      }
  ...
  context "converting SCSS" do
    ...
    it "includes the syntax error line in the syntax error message" do
      ...
      }.to raise_error(Bunto::Converters::Scss::SyntaxError, error_message)
    end
  ...
  context "importing partials" do
    ...
    it "uses a compressed style" do
      ...
      expect(instance.bunto_sass_configuration).to eql({"style" => :compressed})
  ...
  context "importing from external libraries" do
    ...
    context "unsafe mode" do
      let(:site) do
        Bunto::Site.new(site_configuration.merge({
          "source" => sass_lib,
          "sass"   => {
